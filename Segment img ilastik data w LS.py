...
def LSQ(df,image):
    ...
    LSQ_result = tp.refine_leastsq(df, image,diameter=(11,11,9),fit_function="disc",param_val = fixed_params, bounds={'x':(17,26),'y':(17,26),'z':(17,26)})
    # i=0
    # while i<chunksize:
    #     plt.imshow(image[:,:,i])
...
with open('tfrGel10212018A_shearRun10292018f_decon_hv01342_table.csv', mode='r') as csv_file:
    ...
    for row in csv_reader:
        ...
        if line_count == 0: # this is the header row. Ignore this
            print(row)
            line_count += 1
        elif float(row[7])<17 or float(row[7])>432:  #skip row if out of x-bound
            print(row)
            #line_count += 1
        elif float(row[8])<17 or float(row[8])>432:  #skip row if out of y-bound
            print(row)
            #line_count += 1
        elif float(row[9])<24 or float(row[9])>175:  #skip row if out of z-bound
            print(row)
            #line_count += 1
        else:
            i=7
    ...
    ds=xr.Dataset(
        {
            ...
            "LSQ_coords":(["index","dimension"],LSQcoords)
        },
        coords={
...
ds.to_netcdf("saved_chunk_xarray_ilastikv1LSQ.nc")
